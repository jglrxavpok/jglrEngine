...
public class SoundEngine
{
	...
	{
		if(getPlayerTransform() != null) if(getPlayerTransform().hasChanged())
		{
			...
			AL10.alListener(AL10.AL_ORIENTATION, Buffers.createFlippedBuffer(getPlayerTransform().getTransformedRotation().getForward().negative().normalize(), getPlayerTransform().getTransformedRotation().getUp().normalize()));
		}
	}
	...
	public SoundResource loadSound(AbstractResource res)
	{
		...
		if(format == null) throw new RuntimeEngineException("Unsupported sound format: " + ext);
		try
		{
		...
		{
			throw new RuntimeEngineException("Error while loading sound " + res.getResourceLocation().getFullPath(), e);
		}
	}
